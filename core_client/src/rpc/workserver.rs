...
impl WorkClient {
    ...
    fn update(&mut self) -> Result<(), WorkServerDisconnected> {
        loop {
            match self.results_channel.recv_timeout(RECV_TIMEOUT) {
                ...
                Err(RecvTimeoutError::Disconnected) => return Err(WorkServerDisconnected::ReceiveResult),
            };
        }
    }
    ...
    pub fn request_work(&mut self, config: &CoreClientConfig, hash: [u8; 32]) -> Result<(), WorkServerDisconnected> {
        self.ack_request(hash);
        let request = WorkRequest{hash, config: config.clone()};
        ...
        loop {
            ...
            if let Ok(elapsed) = time.elapsed() {
                if elapsed.as_secs() > last_log_time {
                    log::debug!("WorkClient::wait_on(): waiting on work for {}", hex::encode(hash).to_uppercase());
                    last_log_time += 1;
                }
            }
            yield_now().await;
    ...
    fn update(&mut self, finished: &[[u8; 32]]) -> Result<(), WorkServerDisconnected> {
        ...
        loop {
            match self.requests_channel.recv_timeout(RECV_TIMEOUT) {
                ...
                Err(RecvTimeoutError::Disconnected) => return Err(WorkServerDisconnected::ReceiveRequest),
            }
        }
        // Purge duplicates
    ...
    pub async fn start(mut self) -> Result<(), WorkServerDisconnected> {
        ...
        loop {
            ...
            if let Some(request) = self.requests.pop() {
                debug!("WorkServer: getting work for {}", hex::encode(request).to_uppercase());
                let rpc_result = rpc.work_generate(&self.config, request, None).await;
                results.push(WorkResult { hash: request, rpc_result });
                debug!("WorkServer: got work for {}", hex::encode(request).to_uppercase());
            }

            let hashes = results.iter().map(|result| result.hash).collect::<Vec<[u8; 32]>>();
